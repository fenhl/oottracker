        ...
        match dbg!(ClientMessage::read_ws021(&mut stream).await?) {
                ...
                tokio::spawn(async move {
                    let (mut old_cells, mut rx) = {
                        ...
                        let restream = match restreams.get(&restream) {
                            ...
                            None => {
                                let _ = ServerMessage::from_error("no such restream").write_ws021(&mut *sink.lock().await).await; //TODO better error handling
                                return
                            }
                        };
                        let (rx, runner) = match restream.runner(&runner) {
                            ...
                            None => {
                                let _ = ServerMessage::from_error("no such runner").write_ws021(&mut *sink.lock().await).await; //TODO better error handling
                                return
                            }
                        };
                        let cells = layout.cells().into_iter()
                            ...
                            .collect::<Vec<_>>();
                        if ServerMessage::Init(cells.clone()).write_ws021(&mut *sink.lock().await).await.is_err() { return } //TODO better error handling
                        (cells, rx.clone())
                    };
                    while let Ok(()) = rx.changed().await { //TODO better error handling
                        ...
                        for (i, (old_cell, new_cell)) in old_cells.iter().zip(&new_cells).enumerate() {
                            if old_cell != new_cell {
                                if (ServerMessage::Update { cell_id: i.try_into().expect("too many cells"), new_cell: new_cell.clone() }).write_ws021(&mut *sink.lock().await).await.is_err() { return } //TODO better error handling
                            }
                        }
                        old_cells = new_cells;
                    }
            ...
            ClientMessage::ClickRestream { restream, runner, layout, cell_id, right } => {
                ...
                let cell = match layout.cells().get(usize::from(cell_id)) {
                    Some(cell) => cell.id,
                    None => {
                        let _ = ServerMessage::from_error("no such cell").write_ws021(&mut *sink.lock().await).await; //TODO better error handling
                        return Ok(())
                    }
                };
                if right {
            ...
            ClientMessage::SubscribeRaw { room } => {
                ...
                tokio::spawn(async move {
                    ...
                    ServerMessage::InitRaw(old_model.clone()).write_ws021(&mut *sink.lock().await).await?;
                    while let Ok(()) = rx.changed().await {
                        let new_model = get_room(&rooms, room.clone(), |room| room.model.clone()).await?;
                        if old_model != new_model {
                            (ServerMessage::UpdateRaw(&new_model - &old_model)).write_ws021(&mut *sink.lock().await).await?;
                        }
                        old_model = new_model;
                    }
                    Ok::<_, Error>(())
            ...
            ClientMessage::SubscribeRoom { room, layout } => {
                ...
                tokio::spawn(async move {
                    ...
                    ServerMessage::Init(old_cells.clone()).write_ws021(&mut *sink.lock().await).await?;
                    while let Ok(()) = rx.changed().await {
                        ...
                        for (i, (old_cell, new_cell)) in old_cells.iter().zip(&new_cells).enumerate() {
                            if old_cell != new_cell {
                                (ServerMessage::Update { cell_id: i.try_into().expect("too many cells"), new_cell: new_cell.clone() }).write_ws021(&mut *sink.lock().await).await?;
                            }
                        }
                        old_cells = new_cells;
                    }

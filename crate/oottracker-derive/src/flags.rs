...
impl FlagName {
    fn to_ident(&self) -> Ident {
        match self {
            ...
            FlagName::Entrance(from, to) => Ident::new(&format!("ENTRANCE_{}_TO_{}", from.to_string().to_case(Case::ScreamingSnake), to.to_string().to_case(Case::ScreamingSnake)), to.span()),
...
pub(crate) fn flags_list(input: FlagsList) -> TokenStream {
    let FlagsList { vis, struct_token, name, field_ty, num_fields, fields } = input;
...
fn scene_name_to_field(ident: &Ident) -> Ident {
...
impl SceneFieldsKind {
    ...
    fn ty(&self, scene_name: &Ident) -> Ident {
        Ident::new(&format!("{}{}", scene_name, match self {
...
pub(crate) fn scene_flags(input: SceneFlags) -> TokenStream {
    let SceneFlags { vis, struct_token, name, scenes } = input;
    let scene_size = 0x1c;
    ...
    let contents = scenes.iter().map(|Scene { name, .. }| {
        let scene_field = scene_name_to_field(name);
        quote!(#vis #scene_field: #name)
    }).collect_vec();
    ...
    for scene in &scenes {
        let scene_field = scene_name_to_field(&scene.name);
    ...
    let scene_variants = scenes.iter()
        .map(|Scene { idx, name, .. }| quote!(#name = #idx));
    let from_id_arms = scenes.iter()
        .map(|Scene { idx, name, .. }| quote!(#idx => Some(Self::#name)));
    let get_mut_items = scenes.iter()
        .map(|Scene { name, .. }| {
            let scene_field = scene_name_to_field(name);
            quote!(Scene::#name => Some(&mut self.#scene_field))
        });
    let try_from_items = scenes.iter()
        .map(|Scene { idx, name, .. }| {
            ...
            quote!(#scene_field: #name::try_from(&raw_data[#start_idx..#end_idx]).map_err(|()| raw_data.clone())?)
        });
    ...
    let region_arms = scenes.iter().filter_map(|Scene { name, data, .. }| data.iter().filter_map(|data| {
        if let SceneData::RegionName(region_name) = data {
            ...
            Some(match region_name {
                RegionName::One(region_name) => quote!(#scene_name => Ok(Region::#region_name?)),
    ...
    */
    quote! {
        ...
        impl Scene {
            ...
            /*
            pub(crate) fn region(&self, ram: &Ram) -> Result<Region, RegionLookupError> {
                match self.0 {
                    #(#region_arms,)*
                    _ => RegionLookup::new(self.regions()?),
                }
            }
            */ //TODO
        }
            ...
            pub(crate) fn checked(&self, check: &crate::check::Check) -> Option<bool> {
                match check {
                    crate::check::Check::AnonymousEvent(at_check, id) => match &**at_check {
                        ...
                        crate::check::Check::Location(loc) => match (id, &**loc) {
                            #(#location_prereqs,)*
                            _ => None,
                        },
                        _ => None,
